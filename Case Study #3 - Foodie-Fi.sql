...
SELECT plan_id, DATE_TRUNC('month', start_date) AS month_start, COUNT(*)AS distribution
FROM subscriptions
WHERE plan_id = 0
...
SELECT plan_id, EXTRACT (YEAR FROM start_date) AS year_start, COUNT(*)
FROM subscriptions
...
SELECT
    ...
    ROUND((COUNT(*) * 100.0 / (SELECT COUNT(*) FROM subscriptions)), 1) AS percentage_churned
FROM
    subscriptions
WHERE
    plan_id = 4;
...
SELECT
    ...
    total_revenue,
    (total_revenue - LAG(total_revenue) OVER (ORDER BY year, month)) / LAG(total_revenue) OVER (ORDER BY year, month) AS monthly_growth_rate
FROM MonthlyRevenue;
