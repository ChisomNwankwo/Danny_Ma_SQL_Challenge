...
SELECT customer_id, node_id,
	EXTRACT(DAY FROM (end_date::timestamp - start_date::timestamp)) AS days_in_node
FROM customer_nodes
	WHERE end_date <> '9999-12-31'
	ORDER BY 1
)

SELECT ROUND(AVG(days_in_node),0)
	FROM DAYS_IN_NODE;

-- What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
WITH DAYS_IN_NODE AS (
SELECT customer_id, node_id,region_name,
	EXTRACT(DAY FROM (end_date::timestamp - start_date::timestamp)) AS days_in_node
...
PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY days_in_node) AS median_days_in_node,
PERCENTILE_CONT(0.8) WITHIN GROUP(ORDER BY days_in_node) AS "80th_percentile",
PERCENTILE_CONT(0.95) WITHIN GROUP(ORDER BY days_in_node) AS "95th_percentile"
	FROM DAYS_IN_NODE
	GROUP BY 1;
...
WITH MonthlyDeposits AS (
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN txn_type = 'deposit' 
			  THEN txn_date END) AS deposit_count,
        COUNT(DISTINCT CASE WHEN txn_type IN ('purchase', 'withdrawal') 
			  THEN txn_date END) AS purchase_withdrawal_count
    FROM customer_transactions
    GROUP BY 1, 2        
)
...
-- DROP new_txn_amount;
ADD new_txn_amount INTEGER; -- Adjust data type as needed

-- Update the new column with values based on the CASE statement
UPDATE customer_transactions
SET new_txn_amount = CASE WHEN txn_type = 'deposit' 
...
SUM(new_txn_amount) OVER (PARTITION BY customer_id ORDER BY txn_date) 
	AS running_balance
FROM customer_transactions 
	)
SELECT
    M.customer_id,
    M.last_date,
    SUM(CASE WHEN R.txn_date = M.last_date THEN running_balance 
		ELSE 0 END) AS closing_balance
FROM MaxDatePerCustomer M
JOIN RunningBalance R 
...
RunningBalance AS (
SELECT customer_id,txn_date,
SUM(new_txn_amount) OVER (PARTITION BY customer_id 
...
SELECT
    M.customer_id,
	SUM(CASE WHEN R.txn_date = M.first_date 
		THEN running_balance ELSE 0 END) AS opening_balance,
    SUM(CASE WHEN R.txn_date = M.last_date 
		THEN running_balance ELSE 0 END) AS closing_balance,
	((SUM(CASE WHEN R.txn_date = M.last_date 
		  THEN running_balance ELSE 0 END)) - (SUM(CASE WHEN R.txn_date = M.first_date 
												   THEN running_balance ELSE 0 END))) 
	/ (SUM(CASE WHEN R.txn_date <= M.first_date 
		   THEN running_balance ELSE 0 END)) * 100 AS percent_diff_in_balance
FROM MaxMinDatePerCustomer M
JOIN RunningBalance R ON M.customer_id = R.customer_id 
GROUP BY 1
ORDER BY 1
	)
SELECT (
    COUNT(*) FILTER (WHERE percent_diff_in_balance >= 5) * 100.0
) / COUNT(*) AS percentage
FROM BalanceCTE;
...
-- Option 1: Data Allocation Based on End-of-Month Balance
WITH MaxDatePerCustomer AS (
   SELECT customer_id, MAX(txn_date) AS last_date
   FROM customer_transactions
   GROUP BY 1
),
EndOfMonthBalance AS (
   SELECT
      M.customer_id,
      EXTRACT(MONTH FROM last_date) AS month,
      SUM(new_txn_amount) OVER (PARTITION BY M.customer_id 
								ORDER BY txn_date ROWS BETWEEN UNBOUNDED PRECEDING 
								AND CURRENT ROW) AS running_balance
   FROM MaxDatePerCustomer M
   JOIN customer_transactions C
   ON M.customer_id = C.customer_id
   WHERE EXTRACT(MONTH FROM txn_date) = EXTRACT(MONTH FROM last_date)
)
SELECT
   month,
   SUM(running_balance) AS total_allocation
FROM EndOfMonthBalance
GROUP BY 1
ORDER BY 1;

-- so we are having negative figues for data allocation which isn't possible, and this is 
--caused by the negative running balance, so we replace negative figures with zero
...
EndOfMonthBalance AS (
   SELECT
      M.customer_id,
      EXTRACT(MONTH FROM last_date) AS month,
      SUM((CASE WHEN new_txn_amount < 0 THEN 0 ELSE new_txn_amount END)) 
	OVER (PARTITION BY M.customer_id ORDER BY txn_date 
...
WITH MonthlyAverageBalances AS (
   SELECT
      customer_id,
      EXTRACT(MONTH FROM txn_date) AS month,
      AVG((CASE WHEN new_txn_amount < 0 THEN 0 ELSE new_txn_amount END)) 
	OVER (PARTITION BY customer_id ORDER BY EXTRACT(MONTH FROM txn_date) 
		  ROWS BETWEEN 30 PRECEDING AND 1 PRECEDING) AS average_balance
   FROM customer_transactions
)
SELECT
   month,
   ROUND(SUM(average_balance),0) AS total_allocation
FROM MonthlyAverageBalances
GROUP BY 1
ORDER BY 1;
...
SELECT customer_id,txn_date,new_txn_amount,
SUM(new_txn_amount) OVER (PARTITION BY customer_id 
						  ORDER BY txn_date) AS running_balance
FROM customer_transactions 
	)
...
(CASE WHEN (new_txn_amount*1*0.06) < 0 THEN 0 ELSE (new_txn_amount*1*0.06) 
                        END) AS daily_interest
FROM customer_transactions
	),
DailyInterest AS (
	SELECT*,
(new_txn_amount + daily_interest) AS txn_amount_with_interest
FROM Interest
),
DataAllocation AS (
	SELECT*,
SUM((CASE WHEN txn_amount_with_interest < 0 THEN 0 ELSE txn_amount_with_interest END)) 
	OVER (PARTITION BY customer_id,day ORDER BY customer_id,day 
		  ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS daily_data_allocation
FROM DailyInterest
	)
SELECT day,
SUM(daily_data_allocation) AS total_allocation
FROM DataAllocation
GROUP BY 1
ORDER BY 1;
...
(CASE WHEN (new_txn_amount*1*0.06) < 0 THEN 0 ELSE (new_txn_amount*1*0.06) 
                        END) AS daily_interest,
(new_txn_amount + (CASE WHEN (new_txn_amount*1*0.06) < 0 
				   THEN 0 ELSE (new_txn_amount*1*0.06) END)) 
	AS txn_amount_with_interest
FROM customer_transactions
	),
DataAllocation AS (
	SELECT*,
SUM((CASE WHEN txn_amount_with_interest < 0 THEN 0 ELSE txn_amount_with_interest END)) 
	OVER (PARTITION BY customer_id,month ORDER BY customer_id,month 
		  ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_balance
FROM Interest
	)
SELECT month,
